        ...
        <ul class="cart-list" v-for="cart in cartList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              minnum="1"
              class="itxt"
              @change="changeNum('input', $event.target.value * 1, cart)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
...
export default {
  ...
  methods: {
    ...
    changeNum: throttle(async function (type, disNum, cart) {
      ...
      if (disNum !== 0) {
        try {
          await this.$store.dispatch("addcartsuccess/addOrUpdateCart", {
            skuId: cart.skuId,
            skuNum: disNum,
          });
          this.getData();
    ...
    async changeChecked(cart, event) {
      ...
      try {
        await this.$store.dispatch("shopcart/changeCheckedState", {
          skuId: cart.skuId,
          isChecked,
        });
        this.getData();
  ...
  computed: {
    ...
    bigSum() {
      return this.cartList.reduce((pre, cur) => {
        let curValue = 0;
        if(cur.isChecked !==0){
          curValue = cur.skuNum * cur.skuPrice;
        }
        return pre + curValue;
      }, 0);
    },
